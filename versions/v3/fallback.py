"""Fallback engine: the pure python-chess alpha-beta from v1.1, used only if nengine fails.
...
class Searcher:
    ...
    def best_move(self, board: chess.Board, budget_s: float) -> chess.Move:
        ...
        try:
            ...
            # TimeUp can fire between a push and its pop anywhere in the tree, so unwind the
            # board back to the root position before handing a move back.
